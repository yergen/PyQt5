...
class MainWindow(QMainWindow, Ui_MainWindow):
    signal_status = pyqtSignal(str, list) #自定义信号
    def __init__(self, parent=None):
        ...
        self.comboBox_dict = {'相关度':'nothing', '时间':'pubdate', '代码':'stockcode_cat', \
        '升序':'asc', '降序':'desc'}
        self.frame_advanced.hide()#默认隐藏frame
        self.download_info_list = [] #存储要下载的信息，每个元素都是字典形式的，存储了要下载的标题，URL等信息
        ...
        #在lineEdit控件上按下Enter键就可以触发搜索或跳转到页码
    ...
    def filter_df(self, df, filter_title_list=[], filter_content_list=[]):
        ...
        #处理内容返回为None的情况，作用是若没有文章内容返回，则不进行过滤
        filter_content_list = [each + '|None' for each in filter_content_list] 
    ...
    def select_checkBox(self, bool):
        sender = self.sender() #sender()返回的是触发了这个信号的哪个控件
    ...
    def page_go(self, go_type):
        #页面跳转主函数
        if go_type == 'title_down': #触发"下一页"按钮
        ...
        elif go_type == 'title_up': #触发“上一页”按钮
            _temp = self.current_page_num_title
            self.current_page_num_title -= 1
            #如果待跳转的页面真实、有效，则继续；否则不进行跳转
            if 1<= self.current_page_num_title <= self.total_pages_title:
        ...
        #此处位置在page_go函数内部       
            ...
            _temp = self.current_page_num_content
            self.current_page_num_content = int(self.lineEdit_title_page.text())
            if 1<= self.current_page_num_content <=self.total_pages_content:
                self.update_tablewidget_content(page_num = self.current_page_num_content)
            else:
                self.current_page_num_content = _temp
    ...
    def download_pdf(self):
        ...
        if download_thread.isRunning() == True:
            QMessageBox.warning(self, '警告', '检测到下载程序正在运行，请不要重复运行', \
                                            QMessageBox.Yes)
            return None
    ...
    def view_one_new_thread(self, download_path, download_one):
        ...
        if not os.path.isfile(path): #若没有该文件
            try:
    ...
    def show_status(self, type, list_args):
        if type == 'download_status':
            ...
            self.statusBar.showMessage('完成：{0}/{3},正确：{1},错误：{2},本次下载：{4}'.format(\
            count_num, count_right, count_err, count_all, title))
        elif type == 'download_status_err':
            count_num, count_all, count_right, count_err, title = list_args
            self.statusBar.showMessage('重新下载失败：完成：{0}/{3},正确：{1},错误：{2},本次下载：{4}'.format(\
            count_num, count_right, count_err, count_all, title))
        elif type == 'select_status':
            self.statusBar.showMessage('已选择：%d' %len(self.download_info_list))
        elif type == 'change_save_path_status':
    ...
    def change_save_path(self):  
        ...
        if not os.path.isdir(self.download_path):
            os.mkdir(self.download_path)
        directory1 = QFileDialog.getExistingDirectory(self, "选取文件夹", self.download_path)
    ...
    def show_tablewidget(self, dict_data, tableWidget, clear_fore=True):
        ...
        if len(dict_data) > 0:
            ...
            for index in range(len_index):
                ...
                for i in ['<em>',  '</em>']: #<em> </em>是服务器对搜索关键词添加的标记，这里对它们进行剔除
        ...
        tableWidget.setVerticalHeaderLabels([str(i) for i in range(1, len_index+1)]) #设置水平方向的名称
    ...
    def update_tablewidget_title(self, page_num=1):
        ...
        total_pages_title, dict_data_title = get_one_page_data(key_word, \
            fulltext_str_flag = 'false', page_num=page_num, date_start = self.start_time, \
            date_end = self.end_time, sortName=self.sort_name, sortType=self.sort_type)
        #把数据显示在表格上
        if total_pages_title != None:
            ...
            #更新当前页码信息
            self.label_page_info_title.setText('%d/%d' %(self.current_page_num_title, self.total_pages_title))
            
    def update_tablewidget_content(self, page_num=1):
        #更新tableWidget_title
        key_word = self.lineEdit.text()
        #从网络爬虫中获取数据
        total_pages_content, dict_data_content = get_one_page_data(key_word, \
            fulltext_str_flag = 'true', page_num=page_num, date_start = self.start_time, \
            date_end = self.end_time, sortName=self.sort_name, sortType=self.sort_type)
        #把数据显示在表格上
    ...
    def get_dateEdit_time(self, dateEdit):
        ...
        datetime_day = datetime.datetime.strptime(dateEdit_day, '%Y-%m-%d')#把一个时间字符串解析为时间元组
    ...
    @pyqtSlot(QDate)
    def on_dateEdit2_dateChanged(self, date):
        self.end_time = self.get_dateEdit_time(self.dateEdit_2)
...
class WorkThread(QThread):
    ...
    def main_download(self, download_list, download_path, download_status='download_status'):
        ...
        for key_dict in download_list:
            ...
            if os.path.isfile(file_path) == True: #若文件已经存在则默认在下成功
                count_right += 1
                signal_list = [count_num,  count_all, count_right, count_err, title]
                self.signal.emit(download_status, signal_list) #循环结束后发射信号
                continue
            else:
                ...
                try:
                    ...
                    signal_list = [count_num,  count_all, count_right, count_err, title]
                    self.signal.emit(download_status, signal_list) #循环结束后发出信号
                    continue
                f.write(data)
                f.close()
                count_right +=1
